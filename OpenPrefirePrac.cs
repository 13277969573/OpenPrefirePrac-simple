...
public class OpenPrefirePrac : BasePlugin
{
    ...
    public override string ModuleVersion => "0.1.47-single";
    ...
    // Single player mode - simplified data structures
    private CCSPlayerController? _currentPlayer;
    private PlayerStatus? _playerStatus;
    private readonly List<int> _botSlots = new();
    ...
    public void OnClientPutInServerHandler(int slot)
    {
        ...
        if (player.IsBot)
        {
            ...
            {
                // No practice running, kick the bot
                KickBot(slot);
            }
        }
        else
        {
            // For players: Initialize player status
            if (_currentPlayer == null)
            {
                _currentPlayer = player;
                _playerStatus = new PlayerStatus(_defaultPlayerSettings!);
    ...
    public HookResult OnPlayerDisconnect(EventPlayerDisconnect @event, GameEventInfo info)
    {
        ...
        // If current player disconnects, clean up everything
        if (_currentPlayer != null && player.Slot == _currentPlayer.Slot)
        {
            if (_playerStatus != null && _playerStatus.PracticeIndex != -1)
            {
                ExitPrefireMode();
            }
    ...
    public HookResult OnPlayerSpawn(EventPlayerSpawn @event, GameEventInfo info)
    {
        ...
        {
            // For managed bots
            if (_playerStatus != null && _botSlots.Contains(playerOrBot.Slot))
            {
                var targetNo = _playerStatus.Progress;
                var practiceIndex = _playerStatus.PracticeIndex;

                if (targetNo < _playerStatus.EnabledTargets.Count)
                {
                    // If there are more targets to place, move bot to next place
                    _playerStatus.Progress++;

                    AddTimer(0.5f, () => FreezeBot(playerOrBot));

                    AddTimer(0.55f, () => MovePlayer(playerOrBot,
                        _practices[practiceIndex].Targets[_playerStatus.EnabledTargets[targetNo]].IsCrouching,
                        _practices[practiceIndex].Targets[_playerStatus.EnabledTargets[targetNo]].Position,
                        _practices[practiceIndex].Targets[_playerStatus.EnabledTargets[targetNo]].Rotation));

                    // Give bot weapons
                    if (_playerStatus.BotWeapon > 0)
                    {
                        SetMoney(playerOrBot, 0);
                        playerOrBot.RemoveWeapons();
                        switch (_playerStatus.BotWeapon)
                        {
                            case 1:
                ...
                else
                {
                    ...
                    // Kick extra bots if all targets are placed
                    _botSlots.Remove(playerOrBot.Slot);
                    KickBot(playerOrBot.Slot);

                    if (_botSlots.Count == 0 && _currentPlayer != null)
                    {
                        // Practice finished.
                        _currentPlayer.PrintToChat($" {ChatColors.Green}[OpenPrefirePrac] {ChatColors.White}{_translator!.Translate(_currentPlayer, "practice.finish")}");
                        ExitPrefireMode();
                    }
                }
            }
        ...
        else
        {
            ...
            if (_currentPlayer == null || playerOrBot.Slot != _currentPlayer.Slot)
                return HookResult.Continue;

            if (_playerStatus == null || _playerStatus.PracticeIndex < 0)
                return HookResult.Continue;
    ...
    public HookResult OnPlayerDeath(EventPlayerDeath @event, GameEventInfo info)
    {
        ...
        if (playerOrBot.IsBot) 
        {
            if (_playerStatus != null && _botSlots.Contains(playerOrBot.Slot))
            {
                ...
                {
                    // Award the player.
                    if (_currentPlayer.PawnIsAlive && _currentPlayer.Pawn.Value != null)
                    {
                        _currentPlayer.GiveNamedItem("item_assaultsuit");
                        RefillAmmo(_currentPlayer);

                        if (_playerStatus.HealingMethod > 1)
                        {
                            var currentHp = _currentPlayer.Pawn.Value.Health;
                            switch (_playerStatus.HealingMethod)
                            {
                                case 2:
                            ...
                            SetPlayerHealth(_currentPlayer, currentHp);
                        }
                    }

                    // Print progress
                    string tmpPractice = _translator!.Translate(_currentPlayer, "map." + _mapName + "." + _practices[_playerStatus.PracticeIndex].PracticeName);
                    string tmpProgress = _translator!.Translate(_currentPlayer, "practice.progress", _playerStatus.EnabledTargets.Count, _playerStatus.EnabledTargets.Count - targetNo + _botSlots.Count - 1);
                    string content = $"{tmpPractice}\u2029{tmpProgress}";
                    _currentPlayer.PrintToCenter(content);
                }
        ...
        else
        {
            ...
            var practiceIndex = _playerStatus.PracticeIndex;
            var numBots = _botSlots.Count;
            
            if (practiceIndex > -1 && numBots < _practices[practiceIndex].NumBots)
            {
                _playerStatus.Progress = 0;
                AddBot(_currentPlayer, _practices[practiceIndex].NumBots - numBots);
            }
        }
    ...
    public void OnRouteSelect(CCSPlayerController player, ChatMenuOption option)
    {
        ...
        int practiceNo = _playerStatus.LocalizedPracticeNames[option.Text];
    ...
    public void OpenDifficultyMenu(CCSPlayerController player, ChatMenuOption option)
    {
        ...
        _playerStatus.LocalizedDifficultyNames.Clear();

        for (var i = 0; i < 6; i++)
        {
            var tmpLocalizedDifficultyName = _translator.Translate(player, $"difficulty.{i}");
            _playerStatus.LocalizedDifficultyNames.Add(tmpLocalizedDifficultyName, i);
            difficultyMenu.AddMenuOption(tmpLocalizedDifficultyName, OnDifficultyChosen);
        }
    ...
    public void OnDifficultyChosen(CCSPlayerController player, ChatMenuOption option)
    {
        ...
        int difficultyNo = _playerStatus.LocalizedDifficultyNames[option.Text];
    ...
    public void OpenModeMenu(CCSPlayerController player, ChatMenuOption option)
    {
        ...
        _playerStatus.LocalizedTrainingModeNames.Clear();

        for (var i = 0; i < 2; i++)
        {
            var tmpLocalizedTrainingModeName = _translator.Translate(player, $"modemenu.{i}");
            _playerStatus.LocalizedTrainingModeNames.Add(tmpLocalizedTrainingModeName, i);
    ...
    public void OnModeChosen(CCSPlayerController player, ChatMenuOption option)
    {
        ...
        var trainingModeNo = _playerStatus.LocalizedTrainingModeNames[option.Text];
    ...
    private void ExitPrefireMode()
    {
        UnsetPrefireMode();
        
        // Always restore convars in single player mode
        RestoreConvars();
    }
    ...
    private void ResetBots()
    {
        ...
        foreach (var botSlot in _botSlots)
        {
    ...
    private void SetupPrefireMode(CCSPlayerController player)
    {
        ...
        var practiceNo = _playerStatus.PracticeIndex;
        ...
        if (_playerStatus.HealingMethod == 1 || _playerStatus.HealingMethod == 5)
        {
    ...
    {
        List<int> botsToKick = new List<int>(_botSlots);
    ...
    private void GenerateRandomPractice()
    {
        if (_playerStatus == null)
            return;
            
        _playerStatus.EnabledTargets.Clear();
        var practiceNo = _playerStatus.PracticeIndex;
        
        for (var i = 0; i < _practices[practiceNo].Targets.Count; i++)
            _playerStatus.EnabledTargets.Add(i);

        if (_playerStatus.TrainingMode == 0)
        {
            ...
            for (var i = 0; i < numToRemove; i++)
                _playerStatus.EnabledTargets.RemoveAt(rnd.Next(_playerStatus.EnabledTargets.Count));
        }
        // 1: Use all of the targets.
    }

    private void CreateGuidingLine()
    {
        if (_playerStatus == null)
            return;
        ...
        for (int i = 0; i < _practices[practiceNo].GuidingLine.Count - 1; i++)
        {
            ...
            _playerStatus.Beams.Add(beamIndex);
        }
    }

    private void DeleteGuidingLine()
    {
        if (_playerStatus == null)
            return;
            
        for (var i = 0; i < _playerStatus.Beams.Count; i++)
        {
            var beam = Utilities.GetEntityFromIndex<CBeam>(_playerStatus.Beams[i]);
    ...
    private void SetupConvars()
    {
        ...
        AddTimer(2f, () => {
            ...
            foreach(var player in players)
            {
                ...
                if (_botSlots.Contains(player.Slot))
                {
                    continue;
                }
    ...
    private void StartPractice(CCSPlayerController player, int practiceIndex)
    {
        ...
        // Always save and setup convars when starting practice
        SaveConvars();
        SetupConvars();
        AddTimer(0.5f, () => BreakBreakables());

        var previousPracticeIndex = _playerStatus.PracticeIndex;
        ...
        if (previousPracticeIndex != practiceIndex)
        {
            ...
            _playerStatus.PracticeIndex = practiceIndex;
            AddTimer(1f, () => CreateGuidingLine());
        ...
        else
        {
            ...
            var numRemainingBots = _botSlots.Count;
    ...
    private void ChangeMap(CCSPlayerController player, string mapName)
    {
        ...
        // Only allow change map when not practicing
        if (_playerStatus == null || _playerStatus.PracticeIndex < 0)
        {
    ...
    private void ChangeDifficulty(CCSPlayerController player, int difficultyNo)
    {
        ...
        _playerStatus.HealingMethod = difficultyNo;
    ...
    private void ChangeTrainingMode(CCSPlayerController player, int trainingMode)
    {
        ...
        _playerStatus.TrainingMode = trainingMode;
    ...
    private void RegisterCommand()
    {
        ...
        _command = new CommandDefinition("css_prefire", "Command to bring up the main menu of OpenPrefirePrac.", (player, commandInfo) => {
            ...
            // Check if player status is initialized
            if (_playerStatus == null)
            {
                player.PrintToChat($" {ChatColors.Green}[OpenPrefirePrac] {ChatColors.Red}Error: Player status not initialized.");
                return;
            }
            ...
            // 3 Difficulty menu
            string currentDifficulty = _translator.Translate(player, $"difficulty.{_playerStatus.HealingMethod}");
            ...
            string currentTrainingMode = _translator.Translate(player, $"modemenu.{_playerStatus.TrainingMode}");
            ...
            if (_playerStatus.PracticeIndex > 0)
            {
    ...
    private void SetBotWeapon(CCSPlayerController player, int botWeapon)
    {
        ...
        _playerStatus.BotWeapon = botWeapon;
    ...
    private void OpenPracticeMenu(CCSPlayerController player)
    {
        ...
        _playerStatus.LocalizedPracticeNames.Clear();

        // Add menu options for all practices (no conflict checking in single player mode)
        for (var i = 0; i < _practices.Count; i++)
        {
            ...
            _playerStatus.LocalizedPracticeNames.Add(tmpLocalizedPracticeName, i);
    ...
    private void UnsetPrefireMode()
    {
        if (_playerStatus == null)
            return;
            
        var previousPracticeNo = _playerStatus.PracticeIndex;
    ...
    {
        if (_currentPlayer == null || !_currentPlayer.IsValid || _playerStatus == null || _playerStatus.PracticeIndex < 0)
        {
            return;
        }
        ...
        string tmpPractice = _translator!.Translate(_currentPlayer, "map." + _mapName + "." + _practices[_playerStatus.PracticeIndex].PracticeName);
        string tmpProgress = _translator!.Translate(_currentPlayer, "practice.progress", _playerStatus.EnabledTargets.Count, _playerStatus.EnabledTargets.Count - _playerStatus.Progress + _botSlots.Count);
        string content = $"{tmpPractice}\u2029{tmpProgress}";
    ...
    {
        // Remove from bot slots list
        if (_botSlots.Contains(botSlot))
        {
            _botSlots.Remove(botSlot);
        }
    ...
    {
        if (_defaultPlayerSettings!.BotAimLock == 1 && _currentPlayer != null && _currentPlayer.IsValid && _currentPlayer.PawnIsAlive && _currentPlayer.PlayerPawn.Value != null)
        {
            // Aimlock bots to current player
            Vector ownerEyePos = new Vector(_currentPlayer.PlayerPawn.Value.AbsOrigin!.X, _currentPlayer.PlayerPawn.Value.AbsOrigin!.Y, _currentPlayer.PlayerPawn.Value.AbsOrigin!.Z + _currentPlayer.PlayerPawn.Value.ViewmodelOffsetZ);
